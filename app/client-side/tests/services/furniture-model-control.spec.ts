...
import { FurnitureModelControlService } from '../../../client-side/services/furniture-model-control.service';
...
describe('FurnitureModelControlService', () => {
  ...
  describe('GETfurnitureModel', () => {
    it('should return a Blob with model file', async () => {
      ...
      const req = httpMock.expectOne(
        `${BASE_SERVICE_URL}?jwt=${JWT}&furnitureId=${FURNITURE_ID}`
      );
  ...
  describe('POSTuploadFurnitureModel', () => {
    it('should upload model and return message', async () => {
      ...
      const req = httpMock.expectOne(
        `${BASE_SERVICE_URL}?jwt=${JWT}&furnitureId=${FURNITURE_ID}&fileName=model.glb`
      );
