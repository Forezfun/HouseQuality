...
import { FurnitureCardControlService } from '../../client-side/services/furniture-card-control.service';
import {
    BASE_URL,
    JWT,
    FURNITURE_CARD_DATA,
...
describe('FurnitureCardControlService', () => {
    ...
    describe('GETfurnitureCard', () => {
        it('should return furniture card with correct data', async () => {
            const promise = service.GETfurnitureCard('furnitureId', JWT);

            const req = httpMock.expectOne(`${BASE_SERVICE_URL}?furnitureCardId=furnitureId&jwt=${JWT}`);
    ...
    describe('PUTupdateFurnitureCard', () => {
        it('should update furniture card with correct data', async () => {
            const promise = service.PUTupdateFurnitureCard(UPDATED_FURNITURE_CARD_DATA, 'furnitureId', JWT);

            const req = httpMock.expectOne(`${BASE_SERVICE_URL}?jwt=${JWT}&furnitureId=furnitureId`);
    ...
    describe('DELETEfurnitureCard', () => {
        it('should delete furniture card with correct params', async () => {
            const promise = service.DELETEfurnitureCard(JWT, 'furnitureId');

            const req = httpMock.expectOne(`${BASE_SERVICE_URL}?jwt=${JWT}&furnitureCardId=furnitureId`);
